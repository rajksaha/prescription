...
function Show_med1($conn, $appointmentID, $xAxis, $yAxis, $size, $pageNum,$pdf){
        ...
        while($row = $resultData->fetch(PDO::FETCH_ASSOC)){
            ...
            if($drugDoseInitial == "" || $drugDose == ''){
                ...
                $full_str ="($drugDose)$drugDoseInitial $restOftheStringÃ Â¥Â¤";
            }
...
function preparePrescription($conn, $appointmentID){
	...
	$pdf->Line($rightXaxis - 10 , 53, $rightXaxis - 10, 260, $lineStyle);
 	...
 	$rightYaxis = $pdf->Show_med1($conn, $appointmentID,$rightXaxis, $rightYaxis,$size + 2, $pageNum, $pdf);
